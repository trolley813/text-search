...
def get_word_ids(query):
    ...
    for word in words:
        try:
            ...
            similarities = {w: s for w, s in similars}
            similarities[word] = 1.0
        except KeyError:
            similarities = {word: 1.0}
        print(similarities)
        stems = ",".join(f"'{ps.stem(w).replace(quote, quote + quote)}'" for w in similarities)
...
def querysearch(query, doc_threshold=None):
    weights = [1, 10, 100]
    results = []
    con = clickhouse_driver.connect("clickhouse://127.0.0.1")
    ...
    if doc_threshold:
        low, high = doc_threshold
        if len(docs) > high:
            raise ValueError(f"Too many, must be less than {high}, but {len(docs)} found")
        if len(docs) < low:
            raise ValueError(f"Too few, must be more than {low}, but {len(docs)} found")
    doc_count = 0
    for doc in docs:
        doc_count += 1
